...
Parametr $D$ ustawiliœmy na 175 (d³ugoœæ odpowiedzi obiektu na skok jednostkowy). Pierwszym krokiem strojenia by³o przypisanie $N_{\mathrm{u}} = N$, $\lambda = 1$ i stopniowe zmniejszanie parametru $N$ zaczynaj¹c od wielkoœci $D$($Rys. 2.9$). Regulator zacz¹³ dzia³aæ minimalnie gorzej dla $N = 13$($Rys. 2.12$). Od tego momentu ustawiliœmy parametr $N$ na powy¿sz¹ wartoœæ i dobraliœmy najlepsze $N_{\mathrm{u}}$ z zakresu od 1 do $N$. Optymalnie dobre wyniki dawa³ regulator z parametrem $N_{\mathrm{u}} = 2$($Rys. 2.13$). Kolejnym krokiem by³o eksperymentalne zmienianie parametru lambda $\lambda$. Zwiêkszaj¹c lambda powy¿ej 1 otrzymywaliœmy coraz lepsze przebiegi sygna³ów, lecz znacznie odbi³o siê to na szybkoœæ i jakoœæ regulacji. Z kolei w drug¹ stron¹ (zmniejszaj¹c parametr lambda) poprawia³a siê jakoœæ regulacji kosztem przebiegów sygna³ów, które stawa³y siê coraz bardziej chaotyczne i ulega³y gwa³townym, du¿ym zmianom($Rys. 2.14 - Rys. 2.17$). Przyjêliœmy zatem $\lambda = 1$ jako optymaln¹ wartoœæ parametru($Rys. 2.18$).

Nie byliœmy zadowoleni z dzia³ania regulatora z nastawami $N = 13, N_{\mathrm{u}} = 2, \lambda = 1$, przez co zaczêliœmy eksperymentalnie zmieniaæ ponownie parametry w kolejnoœci takiej samej jak poprzednio ($N, N_{\mathrm{u}}, \lambda$) i otrzymaliœmy optymalnie dzia³aj¹cy regulator z nastawami $N = 17, N_{\mathrm{u}} = 1, \lambda = 2$($Rys. 2.19$). WskaŸnik jakoœci regulacji uleg³ lekkiemu pogorszeniu (spadek jakoœci z $E = 8,8069$ na $E = 9,4566$), jednak przebiegi sygna³ów znacznie siê poprawi³y. Proces regulacji przedstawiaj¹ wykresy $Rys. 2.9$ - $Rys. 2.15$.
